...
class Invoice extends Model implements HasMedia
{
    ...
    {
        return $this->hasMany(WarehouseItem::class, 'lr_id');
    }
    ...
    /**
     * Returns the CompanySetting key prefix for this document.
     *
     * Standard invoices use 'invoice'. Transport receipt templates
     * (lr_receipt, lorry_receipt, office_invoice) use their own
     * template_name as the prefix so that per-type Default Formats
     * (email body, address formats, email attachment) are respected.
     */
    protected function documentTypePrefix(): string
    {
        if (in_array($this->template_name, ['lr_receipt', 'lorry_receipt', 'office_invoice'])) {
