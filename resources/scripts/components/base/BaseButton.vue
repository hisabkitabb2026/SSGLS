...
const variantClass = computed<Record<string, boolean>>(() => {
  return {
    ...
    'border-line-default text-body bg-surface hover:bg-hover focus:ring-primary-500 focus:ring-offset-0':
      props.variant == 'white',
    'border-transparent shadow-xs text-white bg-status-red hover:bg-status-red/90 focus:ring-status-red':
      props.variant === 'danger',
    'border-transparent bg-surface-muted border hover:bg-surface-muted/60 focus:ring-line-strong focus:ring-offset-0':
      props.variant === 'gray',
  }
})
