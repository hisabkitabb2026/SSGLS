...
const quotationFields = ref<CustomFieldData[]>([])
const lrReceiptFields = ref<CustomFieldData[]>([])
const lorryReceiptFields = ref<CustomFieldData[]>([])
const officeInvoiceFields = ref<CustomFieldData[]>([])
...
watch(
  ...
  (newValue: CustomFieldData[] | undefined) => {
    ...
    quotationFields.value = data.filter(
      (field) => field.model_type === 'Quotation'
    )
    lrReceiptFields.value = data.filter(
      (field) => field.model_type === 'LrReceipt'
    )
    lorryReceiptFields.value = data.filter(
      (field) => field.model_type === 'LorryReceipt'
    )
    officeInvoiceFields.value = data.filter(
      (field) => field.model_type === 'OfficeInvoice'
    )
    getFields()
  }
)
...
function getFields(): void {
  ...
  if (props.fields.includes('quotation')) {
    fieldList.value.push({
      label: 'Quotation',
      fields: [
        { label: 'Date', value: 'QUOTATION_DATE' },
        { label: 'Expiry Date', value: 'QUOTATION_EXPIRY_DATE' },
        { label: 'Number', value: 'QUOTATION_NUMBER' },
        { label: 'Ref Number', value: 'QUOTATION_REF_NUMBER' },
        ...quotationFields.value.map((i) => ({
          label: i.label,
          value: i.slug,
        })),
      ],
  ...
  if (props.fields.includes('lr_receipt')) {
    fieldList.value.push({
      label: 'LR Receipt',
      fields: [
        { label: 'Date', value: 'LR_RECEIPT_DATE' },
        { label: 'Number', value: 'LR_RECEIPT_NUMBER' },
        { label: 'Ref Number', value: 'LR_RECEIPT_REF_NUMBER' },
        ...lrReceiptFields.value.map((i) => ({
          label: i.label,
          value: i.slug,
        })),
      ],
  ...
  if (props.fields.includes('lorry_receipt')) {
    fieldList.value.push({
      label: 'Lorry Receipt',
      fields: [
        { label: 'Date', value: 'LORRY_RECEIPT_DATE' },
        { label: 'Number', value: 'LORRY_RECEIPT_NUMBER' },
        { label: 'Ref Number', value: 'LORRY_RECEIPT_REF_NUMBER' },
        ...lorryReceiptFields.value.map((i) => ({
          label: i.label,
          value: i.slug,
        })),
      ],
  ...
  if (props.fields.includes('office_invoice')) {
    fieldList.value.push({
      label: 'Office Invoice',
      fields: [
        { label: 'Date', value: 'OFFICE_INVOICE_DATE' },
        { label: 'Number', value: 'OFFICE_INVOICE_NUMBER' },
        { label: 'Ref Number', value: 'OFFICE_INVOICE_REF_NUMBER' },
        ...officeInvoiceFields.value.map((i) => ({
          label: i.label,
          value: i.slug,
        })),
      ],
