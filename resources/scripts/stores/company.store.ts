...
export const useCompanyStore = defineStore('company', () => {
  ...
  async function updateCompanyLogo(data: FormData): Promise<ApiResponse<Company>> {
    try {
      const response = await companyService.uploadLogo(data)

      // The backend returns the updated company (with new logo URL) after upload.
      // Update selectedCompany so the top bar / company switcher reflect the new logo
      // immediately without requiring a page refresh.
      if (response.data) {
        selectedCompany.value = response.data

        // Also update the company in the companies list if it exists there
        const companyIndex = companies.value.findIndex(
          (company) => company.unique_hash === response.data.unique_hash
        )
        if (companyIndex !== -1) {
