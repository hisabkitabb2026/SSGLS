...
export const useGlobalStore = defineStore('global', () => {
  ...
  const menuGroups = computed<MenuItem[][]>(() => {
    ...
    // Filter menu items based on company settings
    const filtered = mainMenu.value.filter(item => {
      const settings = companyStore.selectedCompanySettings

      // Hide estimates if enable_estimate is not YES (default is YES if not set)
      if (item.name === 'estimates') {
        return settings?.enable_estimate !== 'NO'
      }

      // Hide quotations if enable_quotation is not YES (default is YES if not set)
      if (item.name === 'quotations') {
        return settings?.enable_quotation !== 'NO'
      }

      return true
    })

    const sorted = [...filtered].sort((a, b) => (a.priority ?? 100) - (b.priority ?? 100))
