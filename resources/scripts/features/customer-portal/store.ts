...
import type { Estimate, EstimateStatus } from '@/scripts/types/domain/estimate'
...
export interface DashboardData {
  ...
  in_transit?: number
  delivered?: number
  recent_lr_receipts?: any[]
  recent_warehouse_items?: any[]
}
...
export const useCustomerPortalStore = defineStore('customerPortal', {
  ...
  actions: {
    ...
    async fetchInvoices(
      ...
      const { data } = await client.get(
        customerApi(this.companySlug, '/office-invoices'),
        { params },
      )
    ...
    async fetchViewInvoice(id: number | string): Promise<{ data: { data: Invoice } }> {
      const { data } = await client.get(
        customerApi(this.companySlug, `/office-invoices/${id}`),
      )
    ...
    async fetchEstimates(
      ...
      const { data } = await client.get(
        ...
        { params: { ...params, estimate_type: 'quotation' } },
      )
