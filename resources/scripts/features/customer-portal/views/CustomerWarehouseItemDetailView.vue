  ...
  <div>
    ...
    <div class="cp-topbar">
      <div class="cp-topbar-left">
        <h1>Warehouse Item — {{ item?.lr_number || '...' }}</h1>
        <p v-if="item">{{ item.destination_city || '—' }} · {{ warehouseStatusLabel(item.status) }}</p>
      </div>
      <div class="cp-topbar-right">
        <button class="cp-btn cp-btn-outline" @click="goBack">← Back to List</button>
      </div>
    </div>
...
function getTimelineDotClass(status: string): string {
  if (isStatusReached(status)) {
    // If it's the current status, it's "current"; if past, it's "done"
...
async function fetchItem() {
  ...
  try {
    ...
    const { data } = await client.get(`/api/v1/${companySlug}/customer/warehouse-items/${route.params.id}`)
    item.value = data.data || data
  } catch (e) {
    console.error('Failed to fetch warehouse item:', e)
  } finally {
    loading.value = false
  }
}
...
function goBack() {
  router.push(buildCustomerPortalPath(route.params.company as string, 'warehouse-items'))
}
