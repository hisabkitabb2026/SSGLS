  ...
  <BasePage v-else-if="moduleData" class="bg-surface">
          ...
          <template v-else-if="isModuleInstalled">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-1.5">
                <BaseIcon name="CheckCircleIcon" class="h-4 w-4 text-status-green" />
    ...
    <div v-if="moduleData.highlights?.length" class="mt-12 pt-8 border-t border-line-light">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3">
        ...
        >
          <BaseIcon name="CheckIcon" class="h-4 w-4 text-status-green shrink-0 mt-0.5" />
          {{ highlight }}
        </div>
      </div>
    </div>
...
function stepStatusClass(step: InstallationStep): string {
  ...
  switch (status) {
    ...
    case 'running':
      return 'text-status-blue bg-status-blue/10'
    default:
      return 'text-alert-error-text bg-alert-error-bg'
  }
}
