<template>
  <BasePage>
    <BasePageHeader :title="$t('quotations.title')">
      <BaseBreadcrumb>
        <BaseBreadcrumbItem :title="$t('general.home')" to="dashboard" />
        <BaseBreadcrumbItem :title="$t('quotations.estimate', 2)" to="#" active />
      </BaseBreadcrumb>

      <template #actions>
        <BaseButton
          v-show="quotationStore.totalQuotationCount"
        ...
        <router-link v-if="canCreate" to="quotations/create">
          <BaseButton variant="primary" class="ml-4">
            <template #left="slotProps">
              <BaseIcon name="PlusIcon" :class="slotProps.class" />
            </template>
            {{ $t('quotations.new_estimate') }}
          </BaseButton>
        </router-link>
      </template>
    </BasePageHeader>
    ...
    >
      ...
      <BaseInputGroup :label="$t('quotations.status')">
        <BaseMultiselect
      ...
      <div
        class="hidden w-8 h-0 mx-4 border border-line-strong border-solid xl:block"
      ...
      <BaseInputGroup :label="$t('quotations.estimate_number')">
    ...
    <BaseEmptyPlaceholder
      ...
      :title="$t('quotations.no_estimates')"
      :description="$t('quotations.list_of_estimates')"
    >
      <template #actions>
        <BaseButton
          v-if="canCreate"
    ...
    <div v-show="!showEmptyScreen" class="relative table-container">
      ...
      >
        ...
        <BaseDropdown
          v-if="quotationStore.selectedQuotations.length && canDelete"
        ...
        >
          ...
          <BaseDropdownItem @click="removeMultipleQuotations">
      ...
      <BaseTable
        ...
        :placeholder-count="quotationStore.totalQuotationCount >= 20 ? 10 : 5"
        class="mt-4"
      >
        <template #header>
          <div class="absolute items-center left-6 top-2.5 select-none">
            <BaseCheckbox
              v-model="quotationStore.selectAllField"
              variant="primary"
              @change="quotationStore.selectAllQuotations"
            />
          </div>
        </template>
        ...
        <template #cell-estimate_number="{ row }">
          <router-link
            :to="{ name: 'quotations.view', params: { id: row.data.id } }"
        ...
        <template #cell-status="{ row }">
          <BaseQuotationStatusBadge :status="row.data.status" class="px-3 py-1">
            <BaseQuotationStatusLabel :status="row.data.status" />
          </BaseQuotationStatusBadge>
        </template>
...
import QuotationDropdown from '../components/QuotationDropdown.vue'
import SendQuotationModal from '../components/SendQuotationModal.vue'
...
const filters = reactive<QuotationFilters>({
  customer_id: '',
  status: '',
  from_date: '',
  to_date: '',
...
const showEmptyScreen = computed<boolean>(
  () => !quotationStore.totalQuotationCount && !isRequestOngoing.value,
)

const selectField = computed<number[]>({
  get: () => quotationStore.selectedQuotations,
  set: (val: number[]) => {
    quotationStore.selectQuotation(val)
  },
})
...
const quotationColumns = computed<TableColumn[]>(() => [
  {
    key: 'checkbox',
  ...
  { key: 'estimate_number', label: t('quotations.number', 2) },
  { key: 'name', label: t('quotations.customer') },
...
onUnmounted(() => {
  if (quotationStore.selectAllField) {
    quotationStore.selectAllQuotations()
  }
})
...
async function fetchData({ page, sort }: FetchParams): Promise<FetchResult> {
  ...
  const response = await quotationStore.fetchQuotations(data)
...
function removeMultipleQuotations(): void {
  dialogStore.openDialog({
    ...
    if (res) {
      const response = await quotationStore.deleteMultipleQuotations()
      if (response.data) {
        refreshTable()
        quotationStore.$patch((state) => {
          state.selectedQuotations = []
...
function setActiveTab(val: string): void {
  const tabMap: Record<string, string> = {
    ...
    SENT: t('general.sent'),
    VIEWED: t('quotations.viewed'),
    EXPIRED: t('quotations.expired'),
    ACCEPTED: t('quotations.accepted'),
    REJECTED: t('quotations.rejected'),
  }
