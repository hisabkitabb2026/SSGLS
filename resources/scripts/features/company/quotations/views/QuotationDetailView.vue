<template>
  <BasePage v-if="quotationData" class="xl:pl-96 xl:ml-8">
    <BasePageHeader :title="pageTitle">
      <template #actions>
        <div class="mr-3 text-sm">
          <BaseButton
            v-if="quotationData.status === 'DRAFT' && canEdit"
            :disabled="isMarkAsSent"
            :content-loading="isLoadingQuotation"
        ...
        <BaseButton
          v-if="quotationData.status === 'DRAFT' && canSend"
          :content-loading="isLoadingQuotation"
          variant="primary"
          class="text-sm"
          @click="onSendQuotation"
        >
    ...
    >
      ...
      <div
        ref="quotationListSection"
        class="h-full overflow-y-scroll border-l border-line-default border-solid base-scroll"
      >
        <div v-for="(quotation, index) in estimateList" :key="index">
          <router-link
            v-if="quotation"
            :id="'estimate-' + quotation.id"
            :to="`/admin/quotations/${quotation.id}/view`"
            :class="[
          ...
          >
            <div class="flex-2">
              ...
              >
                {{ quotation.estimate_number }}
              </div>
              <BaseQuotationStatusBadge
                :status="quotation.status"
                class="px-1 text-xs"
              >
                <BaseQuotationStatusLabel :status="quotation.status" />
              </BaseQuotationStatusBadge>
            </div>
            ...
            <div class="flex-1 whitespace-nowrap right">
              ...
              >
                {{ quotation.formatted_estimate_date }}
              </div>
            </div>
          </router-link>
        </div>
        ...
        >
          {{ $t('quotations.no_matching_estimates') }}
        </p>
      </div>
    </div>
    ...
    <SendQuotationModal @update="updateSentQuotation" />
  </BasePage>
</template>
...
const estimateData = ref<Quotation | null>(null)
const isMarkAsSent = ref<boolean>(false)
const isLoading = ref<boolean>(false)
const isLoadingQuotation = ref<boolean>(false)

const estimateList = ref<Quotation[] | null>(null)
...
const quotationData = computed(() => estimateData.value)
...
async function loadQuotations(
  ...
  const response = await quotationStore.fetchQuotations({
    page: pageNumber,
    ...params,
  } as never)
  ...
  if (
    ...
    loadQuotations(++currentPageNumber.value)
  }

  if (estimateFound && !fromScrollListener) {
    setTimeout(() => scrollToQuotation(), 500)
  }
}
  ...
  estimateListSection.value?.addEventListener('scroll', (ev) => {
    ...
    if (
      ...
      if (currentPageNumber.value < lastPageNumber.value) {
        loadQuotations(++currentPageNumber.value, true)
      }
    }
  })
}
...
async function loadQuotation(): Promise<void> {
  ...
  const response = await quotationStore.fetchQuotation(Number(route.params.id))
  if (response.data) {
    isLoadingQuotation.value = false
    estimateData.value = { ...response.data.data } as Quotation
  }
}
...
function onMarkAsSent(): void {
  dialogStore.openDialog({
    ...
    if (res) {
      ...
      await quotationStore.markAsSent({
        id: estimateData.value!.id,
        status: 'SENT',
      })
      estimateData.value!.status = 'SENT' as Quotation['status']
...
function updateSentQuotation(): void {
  ...
  if (pos !== -1 && estimateList.value[pos]) {
    estimateList.value[pos].status = 'SENT' as Quotation['status']
    estimateData.value!.status = 'SENT' as Quotation['status']
  }
}
