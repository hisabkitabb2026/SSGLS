...
async function submitCustomerData(): Promise<void> {
  ...
  try {
    ...
    if (response.data) {
      ...
      // Check openContext to know which selector opened the modal:
      //   - 'consignee' → dispatch event so InvoiceBasicFields auto-selects
      //     the new customer as the consignee (not the main customer)
      //   - 'customer' or null → auto-select as main customer (default behavior)
      if (customerStore.openContext === 'consignee') {
        ...
        // General case: dispatch event so BaseCustomerSelectInput (and any
        // other listener) can auto-select the newly created customer.
        window.dispatchEvent(
