...
export const useCustomerStore = defineStore('customer', () => {
  ...
  /**
   * Tracks which selector opened the customer/party modal so the modal
   * knows where to auto-select the newly created customer.
   *   - 'customer' → auto-select as main customer (default behavior)
   *   - 'consignee' → auto-select as consignee (LR Receipt)
   *   - null → no auto-select context (backward compat)
   */
  const openContext = ref<'customer' | 'consignee' | null>(null)
