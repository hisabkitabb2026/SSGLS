 ...
 * the customer list hides invoice-specific columns like "Amount Due"
 * and shows "New Party" instead of "New Customer".
 */
...
const pageTitle = computed<string>(() => {
  return isPartyMode.value ? 'Parties' : t('customers.title')
})
...
const breadcrumbLabel = computed<string>(() => {
  ...
  return t('customers.customer', 2)
})

const emptyPlaceholderTitle = computed<string>(() => {
  return isPartyMode.value ? 'No parties' : t('customers.no_customers')
})

const emptyPlaceholderDescription = computed<string>(() => {
  return isPartyMode.value ? 'List of parties' : t('customers.list_of_customers')
})
      ...
      <template #actions>
        <div class="flex items-center justify-end space-x-5">
          ...
          >
            ...
            {{ isPartyMode ? $t('customers.new_party') : $t('customers.new_customer') }}
          </BaseButton>
        </div>
      </template>
    </BasePageHeader>
    ...
    >
      ...
      <template #actions>
        ...
        >
          ...
          {{ isPartyMode ? $t('customers.new_party') : $t('customers.add_new_customer') }}
        </BaseButton>
      </template>
    </BaseEmptyPlaceholder>
