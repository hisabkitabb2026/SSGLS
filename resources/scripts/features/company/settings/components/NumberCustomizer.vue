...
/**
 * NumberCustomizer — Drag-and-drop number format builder.
 *
 * Refactored:
 * - Removed hardcoded English strings → all i18n via settings-registry keys
 * - Uses useSettings composable instead of direct companyStore access
 * - Accepts settingKey prop for document-type-specific formats
 */
...
// ── i18n-driven labels (no more hardcoded English) ──
// For document types that use a custom settingKey (LR, Lorry, Office Invoice),
// we use the settingKey to look up the correct i18n label.
const formatTitle = computed<string>(() => {
  const keyMap: Record<string, string> = {
    lr_receipt_number_format: t('settings.customization.lr_receipts.lr_receipt_number_format'),
    lorry_receipt_number_format: t('settings.customization.lorry_receipts.lorry_receipt_number_format'),
    office_invoice_number_format: t('settings.customization.office_invoices.office_invoice_number_format'),
    invoice_number_format: t(`settings.customization.${props.type}s.${props.type}_number_format`),
    estimate_number_format: t(`settings.customization.${props.type}s.${props.type}_number_format`),
    payment_number_format: t(`settings.customization.${props.type}s.${props.type}_number_format`),
    quotation_number_format: t(`settings.customization.${props.type}s.${props.type}_number_format`),
  }
...
const formatDescription = computed<string>(() => {
  const keyMap: Record<string, string> = {
    lr_receipt_number_format: t('settings.customization.lr_receipts.lr_receipt_number_format_description'),
    lorry_receipt_number_format: t('settings.customization.lorry_receipts.lorry_receipt_number_format_description'),
    office_invoice_number_format: t('settings.customization.office_invoices.office_invoice_number_format_description'),
    ...
    quotation_number_format: t(`settings.customization.${props.type}s.${props.type}_number_format_description`),
  }
...
const previewLabel = computed<string>(() => {
  const keyMap: Record<string, string> = {
    lr_receipt_number_format: t('settings.customization.lr_receipts.preview_lr_receipt_number'),
    lorry_receipt_number_format: t('settings.customization.lorry_receipts.preview_lorry_receipt_number'),
    office_invoice_number_format: t('settings.customization.office_invoices.preview_office_invoice_number'),
    invoice_number_format: t(`settings.customization.${props.type}s.preview_${props.type}_number`),
    estimate_number_format: t(`settings.customization.${props.type}s.preview_${props.type}_number`),
    payment_number_format: t(`settings.customization.${props.type}s.preview_${props.type}_number`),
    quotation_number_format: t(`settings.customization.${props.type}s.preview_${props.type}_number`),
  }
...
async function submitForm(): Promise<boolean> {
  ...
  data[formatKey] = getNumberFormat.value

  await saveSettings(
    data,
    `settings.customization.${props.type}s.${props.type}_settings_updated`,
  )
