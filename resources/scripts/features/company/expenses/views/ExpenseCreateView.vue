      ...
      <BaseCard>
        <BaseInputGrid>
          ...
          >
            ...
            >
              <template #action>
                <BaseSelectAction @mousedown.prevent="openCategoryModal">
                  <BaseIcon
                    name="PlusIcon"
                    class="h-4 mr-2 -ml-2 text-center text-primary-400"
                  />
                  {{ $t('expenses.add_new_category') }}
                </BaseSelectAction>
              </template>
            </BaseMultiselect>
          </BaseInputGroup>
...
const customerLabel = computed<string>(() => {
  return isPartyMode.value ? 'Party' : t('expenses.customer')
})

const customerPlaceholder = computed<string>(() => {
  return isPartyMode.value ? 'Select a party' : t('customers.select_a_customer')
})
...
function openCategoryModal(): void {
  modalStore.openModal({
    title: t('expenses.add_new_category'),
    componentName: 'CategoryModal',
    refreshData: reloadCategories,
    size: 'sm',
  })
}

async function reloadCategories(): Promise<void> {
  // Reload the category dropdown after a new category is created
  // This will be called when the modal is closed after successful category creation
}
...
async function submitForm(): Promise<void> {
  ...
  try {
    ...
    // Refresh dashboard data after expense save
