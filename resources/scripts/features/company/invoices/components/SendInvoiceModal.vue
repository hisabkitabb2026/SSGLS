...
async function setInitialData(): Promise<void> {
  try {
    ...
    // Use template-specific mail body for transport receipt templates
    const templateName = modalData.value?.template_name as string | undefined
    const mailBodyKey = templateName && ['lr_receipt', 'lorry_receipt', 'office_invoice'].includes(templateName)
      ? `${templateName}_mail_body`
      : 'invoice_mail_body'
    form.body = companyStore.selectedCompanySettings[mailBodyKey] ?? null
