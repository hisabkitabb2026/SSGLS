<template>
  <BasePage>
    ...
    <BaseFilterWrapper
      ...
      @clear="clearFilter"
    >
      <BaseInputGroup :label="filterCustomerLabel">
...
const customerColumnLabel = computed<string>(() => {
  return isPartyMode.value ? 'Party' : t('invoices.customer')
})

const filterCustomerLabel = computed<string>(() => {
  return isPartyMode.value ? 'Party' : t('customers.customer', 1)
})
...
const invoiceColumns = computed<TableColumn[]>(() => [
  ...
  { key: 'name', label: customerColumnLabel.value },
...
const recurringColumns = computed<TableColumn[]>(() => [
  ...
  { key: 'customer', label: customerColumnLabel.value },
