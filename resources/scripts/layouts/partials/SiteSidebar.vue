      ...
      >
        <div class="relative flex flex-col flex-1 w-full max-w-xs bg-surface">
          ...
          <div class="flex-1 h-0 pt-5 pb-4 overflow-y-auto">
            ...
            <nav
              v-for="(menu, index) in reorderedMenuGroups"
              :key="index"
  ...
  >
    ...
    >
      ...
      <router-link
        v-for="item in menu"
        :key="item.name"
        v-tooltip="globalStore.isSidebarCollapsed ? { content: getMenuLabel(item.title), placement: 'right' } : null"
        :to="item.link"
        :class="[
          hasActiveUrl(item.link)
            ? 'text-primary-600 bg-primary-50 font-semibold'
...
const filteredMenuGroups = computed(() => {
  // Filter menu groups based on company settings
  return globalStore.menuGroups.map(menu =>
    menu.filter(item => {
      const settings = companyStore.selectedCompanySettings
      // Hide estimates if enable_estimate is 'NO'
      if (item.name === 'estimates' && settings?.enable_estimate === 'NO') {
        return false
      }
      // Hide quotations if enable_quotation is 'NO'
      if (item.name === 'quotations' && settings?.enable_quotation === 'NO') {
        return false
      }
      // Hide Warehouse Operations if enable_warehouse_items is 'NO'
      if (item.name === 'Warehouse Operations' && settings?.enable_warehouse_items === 'NO') {
        return false
      }
      // Hide Fleet Management if enable_fleet_management is 'NO'
      if (item.name === 'Fleet Management' && settings?.enable_fleet_management === 'NO') {
        return false
      }
      // Hide Customers if enable_customers is 'NO'
      if (item.name === 'Customers' && settings?.enable_customers === 'NO') {
        return false
      }
      // Hide Items if enable_items is 'NO'
      if (item.name === 'Items' && settings?.enable_items === 'NO') {
        return false
      }
      // Hide Dashboard if enable_dashboard is 'NO'
      if (item.name === 'Dashboard' && settings?.enable_dashboard === 'NO') {
        return false
      }
      // Hide Payments if enable_payments is 'NO'
      if (item.name === 'Payments' && settings?.enable_payments === 'NO') {
        return false
      }
      // Hide Expenses if enable_expenses is 'NO'
      if (item.name === 'Expenses' && settings?.enable_expenses === 'NO') {
        return false
      }
      // Hide Reports if enable_reports is 'NO'
      if (item.name === 'Reports' && settings?.enable_reports === 'NO') {
        return false
      }
      // Hide Members if enable_members is 'NO'
      if (item.name === 'Members' && settings?.enable_members === 'NO') {
        return false
      }
      return true
    })
  ).filter(menu => menu.length > 0) // Remove empty groups
})

// Reorder menu groups so Dashboard always appears at the top of the sidebar
const reorderedMenuGroups = computed(() => {
  // Create a shallow copy to avoid mutating the filtered computed
  const groups = [...filteredMenuGroups.value]

  const dashboardGroupIndex = groups.findIndex(group =>
    group.some(item => item.name === 'Dashboard')
  )

  if (dashboardGroupIndex > 0) {
    // Move the Dashboard group to the top
    const [dashboardGroup] = groups.splice(dashboardGroupIndex, 1)
    groups.unshift(dashboardGroup)
  }
...
function getMenuLabel(title: string): string {
  if (!isPartyMode.value) {
    return t(title)
  }
  // Replace 'navigation.customers' with 'Party' when in Party Mode
  if (title === 'navigation.customers') {
    return 'Party'
  }
  return t(title)
}
...
const activeMenuLink = computed<string | null>(() => {
  const allLinks = filteredMenuGroups.value.flat().map((item) => item.link)
